...
class EraserSetterRGB(object):
    ...
    def __init__(self, config):
        # 从yaml文件读取配置信息
        # min_overlap 是擦除器与图像的最小重叠比例
        self.min_overlap = config['min_overlap']
        # max_overlap 是擦除器与图像的最大重叠比例
        self.max_overlap = config['max_overlap']
        # min_cut_ratio 是擦除器与图像的最小切割比例
        self.min_cut_ratio = config['min_cut_ratio']
        # max_cut_ratio 是擦除器与图像的最大切割比例
        self.max_cut_ratio = config.get('max_cut_ratio', 1.0) # 使用 get 方法提供默认值 1.0
